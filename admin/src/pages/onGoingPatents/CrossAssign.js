...
const DynamicPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchJobData = async (id) => {
      try {
        ...
        const response = await axios.get(`admin/job_order/${id}`);
  ...
  const onClickFindPartner = async (Service, getCountry) => {
    try {
      ...
      setClicked(true);
      const response = await axios.get(`cross-assign/find-partner/${Service}/${getCountry}/${prevPartner}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const assignResponse = await axios.post("/cross_assign", {
        JobID: id,
