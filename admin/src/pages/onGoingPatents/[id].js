...
function getStatusColor(status) {
  if (status === 'In Progress') {
    return {
      background:
        'rgba(255, 255, 0, 0.1)' /* Yellow background with reduced opacity */,
      borderRadius: '4px',
      fontWeight: 'bold',
...
const DynamicPage = () => {
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      try {
        const response = await fetch('/api/admin/job_order')
    ...
    const fetchJobFileData = async (jobID) => {
      try {
        const response = await axios.get(
          `/api/admin/job_files_details/${jobID}`
        )
  ...
  const onClickDownload = async (jobId) => {
    console.log(access)
    try {
      const response = await axios.get(`/api/admin/job_files/${jobId}`)
          ...
          >
            <tbody>
              ...
              <tr>
                ...
                >
                  <span style={getStatusColor(status)}>{status}</span>
                </td>
                <td
                  style={{
                    padding: '10px',
                ...
                >
                  <Button
                    sx={{
                      ...
                      '&:hover': {
                        background:
                          'linear-gradient(90deg, #5F9EA0 0%, #7FFFD4 100%)'
                      }
                    }}
          ...
          {downloadStatus && !access && (
            <Grid container spacing={2}>
              ...
              >
                ...
                >
                  Partner's Work Access Management
                </h2>
              </Grid>
            </Grid>
          )}
        ...
        <CenteredDialogActions>
          ...
          <DialogActions>
            <ColorButton
              ...
              sx={{ width: '15%' }}
              onClick={() => {
                onPartnerNotif(job.og_id, reasons)
                window.location.reload(true)
              }}
            >
              Submit
