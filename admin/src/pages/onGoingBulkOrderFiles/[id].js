...
const DynamicPage = () => {
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      try {
        const response = await axios.get(`/that-bulk-order-file/${id}`);
    ...
    const fetchOnlyFiles = async () => {
      try {
        const response = await axios.get(`only-that-bulk-order-file/${id}`);
  ...
  const handleServiceSelection = async (event) => {
    ...
    if (event.target.value != '') {
      try {
        const response = await axios.get(`admin/bulk-order/partner/${event.target.value}`);
