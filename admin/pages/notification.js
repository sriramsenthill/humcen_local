...
export default function NotificationTable() {
  ...
  useEffect(() => {
    const fetchNotifData = async () => {
      try {
        const notifResponse = await axios.get(`admin/get-notifs`);
  ...
  const clickedByUser = async (notifId) => {
    const clickConfirm = await axios.put(`admin/seen-notif/${notifId}`, { seen: true })
    clickConfirm.then(() => {
  ...
  const deleteSelectedNotifs = async (deleteTheseNotifs) => {
    ...
    const response = await axios.put(`admin/delete-notif`, { deletedNotifs: selectedNotifs }).then(() => {
  ...
  const handleDaysSort = async (e, userID) => {
    ...
    const sortedNotif = await axios.get(`admin/sort-notif/${timeInterval}`);
