...
const BulkOrderComponent = () => {
  ...
  const createBulkOrders = async (jobs, titles, services, files, user) => {
    ...
    const eventSource = new EventSource('/api/user/create-bulk-orders-progress');
    eventSource.addEventListener('progress', (event) => {
      const progress = parseFloat(event.data);
      console.log(progress);
    });

    const response = await axios.post(`user/create-bulk-orders/`, {
      bulkJobs: jobs,
      bulkTitles: titles,
  ...
  const runPythonScript = async (csvBase64) => {
    try {
      const response = await axios.get(`process-base64-csv/${csvBase64}`);
