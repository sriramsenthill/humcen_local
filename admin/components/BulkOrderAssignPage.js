...
const BulkOrderAssignPage = ({ detailsList, jobLists, services, countries }) => {
  ...
  const handleBulkAssigns = async (partnerID, allJobs) => {
    try {
      setClicked(true);
      const sendPartnerData = await axios.post(`bulk-orders/assign/${partnerID}/${allJobs}`)
  ...
  useEffect(() => {
    const fetchPartners = async (allJobs) => {
      try {
        const partners = await axios.get(`find-partners/bulk-orders/${services}/${countries}/${allJobs}`);
        if (partners.data) {
