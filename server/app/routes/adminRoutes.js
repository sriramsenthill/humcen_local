...
adminRouter.use(function (req, res, next) {
  ...
  try {
    ...
    logger.error('Failed to admin verify token:', error);
    return res.status(401).json({});
  }
});
...
adminRouter.get("/Unassigned/only-details/:jobID", data.getUnassignedJobDetailsById); // Getting only the Necessary Details
...
adminRouter.get("/user_files/:services/:id", data.getUnassignedJobFilesForAdmin); // To fetch Unassigned User Files for Admin

adminRouter.get("/cross-assign/find-partner/:services/:country/:partID", data.getPartnersDataForCrossAssign); // Fetching out Available Partners to Cross Assign the Task
...
adminRouter.get("/bulk-order/partner/:service/:country", data.getPartnersForBulkOrder); // Getting Partner's Details according to the service chosen by Admin
...
adminRouter.get("/that-bulk-order-file/:fileNo", data.getParticularBulkOrderFileDetails); // This one fetches details from Bulk Order Files schema

adminRouter.get("/only-that-bulk-order-file/:fileNo", data.getOnlyTheParticularBulkOrderFile); // This one fetches only files from Bulk Order Files schema

adminRouter.get("/bulk-assign-details/:bulkLists", data.getBulkOrderAssignTabDetails); // Bulk Order details for Bulk Assign

adminRouter.get("/find-partners/bulk-orders/:service/:country/:bulkOrders", data.getBulkOrderAssignPartners); // Finding Partner details for Bulk Assign

adminRouter.post("/bulk-orders/assign/:partnerID/:bulkIDs", data.assignBulkOrdersToPartners) // Sending Assign Results for Partner Assign
...
adminRouter.put("/applicant-settings", adminSettings.updateAdminApplicantDetails) // For Updating Admin's Applicant Details Settings
