...
export default function NotificationTable() {
  ...
  useEffect(() => {
    if (userID) {
      const fetchNotifData = async () => {
        try {
          const notifResponse = await axios.get(`user/get-notifs/${userID}`);
  ...
  const clickedByUser = async (notifId, userID) => {
    const clickConfirm = await axios.put(`seen-notif/${userID}/${notifId}`, { seen: true })
    clickConfirm.then(() => {
  ...
  const deleteSelectedNotifs = async (userID, deleteTheseNotifs) => {
    ...
    const response = await axios.put(`delete-notif/${userID}`, { deletedNotifs: selectedNotifs }).then(() => {
  ...
  const handleDaysSort = async (e, userID) => {
    ...
    const sortedNotif = await axios.get(`sort-notif/${userID}/${timeInterval}`);
